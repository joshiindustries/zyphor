...
Zyphor accepts up to 10 files of 50 MB each. When deploying behind Nginx, configure
`client_max_body_size 512m;` for the upload route (the included Docker Compose proxy
already does this). A `413 Request Entity Too Large` response is generated by a proxy
or hosting platform before the Next.js upload route runs; it is not an encryption error.

### Production security settings

For `zyphorr.vercel.app`, set `NEXT_PUBLIC_TURNSTILE_ENABLED=true`,
`TURNSTILE_ENFORCE=true`, `TURNSTILE_SECRET_KEY`, and
`TURNSTILE_EXPECTED_HOSTNAME=zyphorr.vercel.app` in Vercel. Tokens are verified with
Cloudflare on the server for credential login and registration; a client-supplied token
alone is never trusted. Public transfer links remain available to recipients, but are
rate-limited. Uploading and account/data APIs require a signed-in user.

### Jenkins CI

The repository includes a declarative [`Jenkinsfile`](./Jenkinsfile). Create a
**Multibranch Pipeline** in Jenkins pointed at this repository and give its agents
Node.js 18+ and Docker access. Every branch is installed, type-checked, and built.
Set `PUSH_IMAGE=true` only after adding a Jenkins **Username with password** credential
for your container registry, then set its ID in `REGISTRY_CREDENTIALS_ID` and enter the
registry/repository values. CI deliberately never runs `prisma db push`; schema changes
must be deployed through reviewed Prisma migrations in the runtime deployment.
