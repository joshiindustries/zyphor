...
export default function DesktopLoginPage() {
  ...
  useEffect(() => {
    ...
    const redirectUri = query.get("redirect_uri"); const state = query.get("state"); const codeChallenge = query.get("code_challenge");
    if (query.get("social") !== "1" || !redirectUri || !state || !codeChallenge) return;
    setLoading(true); setError("");
    fetch("/api/desktop/auth/complete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ redirectUri, codeChallenge }) })
      .then(async response => ({ response, data: await response.json() }))
      .then(({ response, data }) => { if (!response.ok || !data?.code) throw new Error(data?.error || "Social sign-in did not complete."); completeWithCode(data.code, redirectUri, state); })
      .catch(cause => { setError(cause instanceof Error ? cause.message : "Social sign-in did not complete."); setLoading(false); });
  }, []);

  function social(provider: "google" | "github") {
    const query = new URLSearchParams(window.location.search); query.set("social", "1");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (redirectUri && state && data.code) { completeWithCode(data.code, redirectUri, state); return; }
  ...
  return <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#0a0a0f", color: "#f0f0f5", fontFamily: "Segoe UI, sans-serif", padding: "24px" }}>
    <form onSubmit={submit} style={{ width: "100%", maxWidth: 420, background: "#13131c", padding: 32, borderRadius: 16, border: "1px solid #2a2a35", display: "grid", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><button type="button" disabled={loading} onClick={() => social("google")} style={{ padding: 12, borderRadius: 8, border: 0, fontWeight: 700 }}>Google</button><button type="button" disabled={loading} onClick={() => social("github")} style={{ padding: 12, borderRadius: 8, border: "1px solid #39404b", color: "white", background: "#24292e", fontWeight: 700 }}>GitHub</button></div>
